...
int registro() //Função para registrar usuários
{
	...
	char arquivo[40]; 			//início da criação das variáveis em string
	char cpf[40];
	char nome[40];
	char sobrenome[40];
	char cargo[40];				//final das variávies string
	...
	file = fopen(arquivo, "w"); // cria o arquivo e o "w" significa, dentro da linguagem, "escrever"
...
int consulta()
{
	...
	if(file == NULL)
	{
		printf("Não foi possível localizar o CPF digitado; \n");
	}
